<?php
include('header.php');

if (isset($_GET) && isset($_GET['id'])) {
    $id = $_GET['id'];
    $qry = "SELECT * FROM properties WHERE id=$id";
    $result = $conn->query($qry);




    if ($result->num_rows > 0) {
        // output data of each row
        while ($row = $result->fetch_assoc()) {
?>
            <div class="clearfix"></div>
            <!-- Header Container / End -->
            <!-- END SECTION HEADINGS -->

            <!-- START SECTION PROPERTIES LISTING -->
            <section class="single-proper blog details">
                <div class="container">
                    <div class="row">
                        <section class="col-12 px-3 headings-2 pt-0">
                            <div class="pro-wrapper">
                                <div class="detail-wrapper-body">
                                    <div class="listing-title-bar">
                                        <h3>Property ID: <?php echo $row['property_id'] ?></h3>
                                        <div class="mt-0">
                                            <!-- <a href="#listing-location" class="listing-address">
                                                <i class="fa fa-map-marker pr-2 ti-location-pin mrg-r-5"></i><?php echo $row['address'] ?>
                                            </a> -->
                                        </div>
                                    </div>
                                </div>
                                <div class="single detail-wrapper mr-2">
                                    <div class="detail-wrapper-body">
                                        <div class="listing-title-bar">
                                            <!-- <h4>$ 230,000</h4> -->
                                            <div class="mt-0">
                                                <a href="#listing-location" class="listing-address">
                                                    <!-- <p>$ 1,200 / sq ft</p> -->
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </section>
                        <div class="col-lg-8 col-md-12 blog-pots">
                            <div class="row">
                                <div class="col-md-12">
                                    <!-- main slider carousel items -->
                                    <div id="listingDetailsSlider" class="carousel listing-details-sliders slide mb-30">
                                        <h5 class="mb-4">Gallery</h5>
                                        <div class="carousel-inner">
                                            <?php
                                            $images = json_decode($row['imageUrls']);
                                            $count =  count($images);
                                            $i = 0;
                                            while ($i < $count) {
                                            ?>
                                                <div class="<?Php if ($i === 0) {
                                                                echo 'active';
                                                            } ?> item carousel-item" data-slide-number="<?php echo $i; ?>">
                                                    <img src="<?php echo $images[$i]; ?>" class="img-fluid" alt="slider-listing">
                                                </div>
                                            <?php
                                                $i++;
                                            }

                                            ?>

                                            <a class="carousel-control left" href="#listingDetailsSlider" data-slide="prev"><i class="fa fa-angle-left"></i></a>
                                            <a class="carousel-control right" href="#listingDetailsSlider" data-slide="next"><i class="fa fa-angle-right"></i></a>

                                        </div>
                                        <!-- main slider carousel nav controls -->
                                        <ul class="carousel-indicators smail-listing list-inline">
                                            <?php
                                            $images = json_decode($row['imageUrls']);
                                            $count =  count($images);
                                            $i = 0;
                                            while ($i < $count) {
                                            ?>
                                                <li class="list-inline-item active">
                                                    <a id="carousel-selector-0" class="selected" data-slide-to="<?php echo $i; ?>" data-target="#listingDetailsSlider">
                                                        <img src="<?php echo $images[$i]; ?>" class="img-fluid" alt="listing-small">
                                                    </a>
                                                </li>
                                            <?php
                                                $i++;
                                            }

                                            ?>

                                        </ul>
                                        <!-- main slider carousel items -->
                                    </div>
                                    <!-- <div class="blog-info details mb-30">
                                        <h5 class="mb-4">Description</h5>
                                        <p><?php
                                            $floorplans = json_decode($row['floorPlans']);
                                            // echo print_r($floorplans);


                                            // echo "<br/> " . $min . "<br/> " . $max;
                                            ?></p>
                                    </div> -->
                                </div>
                            </div>

                            <div class="floor-plan property wprt-image-video w50 pro">
                                <h5 class="col-12">Floor Plans</h5>
                                <ul class="nav nav-pills mb-3 col-12" id="pills-tab" role="tablist">
                                    <?php
                                    // echo print_r($floorplans); 
                                    $lavel = '';
                                    foreach ($floorplans as $floor) {
                                        // echo print_r($floor) . '<br/><br/>';
                                        // continue;

                                        $units = $floor->units;
                                        if (count($units) > 0) {
                                            if ($floor->bed === 'studio') {
                                                $lavel = '<li class="nav-item col-3 text-center px-0">
                                        <a class="nav-link" id="pills-home-tab" data-toggle="pill" href="#pills-home" role="tab" aria-controls="pills-home" aria-selected="true">Studio</a></li>';
                                            }
                                        }
                                    }
                                    echo $lavel;
                                    $lavel = '';
                                    foreach ($floorplans as $floor) {
                                        // echo print_r($floor) . '<br/><br/>';
                                        $units = $floor->units;
                                        if (count($units) > 0) {
                                            if ($floor->bed === 'bedroom1') {
                                                $lavel = ' <li class="nav-item col-3 text-center px-0">
                                            <a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#pills-profile" role="tab" aria-controls="pills-profile" aria-selected="false">1 Bed</a>
                                        </li>';
                                            }
                                        }
                                    }
                                    echo $lavel;
                                    $lavel = '';
                                    foreach ($floorplans as $floor) {
                                        // echo print_r($floor) . '<br/><br/>';
                                        $units = $floor->units;
                                        if (count($units) > 0) {
                                            if ($floor->bed === 'bedroom2') {
                                                $lavel = '<li class="nav-item col-3 text-center px-0">
                                            <a class="nav-link" id="pills-contact-tab" data-toggle="pill" href="#pills-contact" role="tab" aria-controls="pills-contact" aria-selected="false">2 Bed</a>
                                            </li>';
                                            }
                                        }
                                    }
                                    echo $lavel;
                                    $lavel = '';
                                    foreach ($floorplans as $floor) {
                                        // echo print_r($floor) . '<br/><br/>'
                                        $units = $floor->units;
                                        if (count($units) > 0) {;
                                            if ($floor->bed === 'bedroom3') {
                                                $lavel = '<li class="nav-item col-3 text-center px-0">
                                            <a class="nav-link" id="pills-abcd-tab" data-toggle="pill" href="#pills-abcd" role="tab" aria-controls="pills-abcd" aria-selected="false">3 Bed</a>
                                        </li>';
                                            }
                                        }
                                    }
                                    echo $lavel;
                                    ?>
                                </ul>
                                <div class="tab-content col-12 p-md-3 p-1" id="pills-tabContent">
                                    <div class="tab-pane" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                                        <?php
                                        // echo print_r($floorplans); 
                                        $empty_plans = array();
                                        foreach ($floorplans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';

                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'studio') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }
                                                    if (empty($unit_counting)) {
                                                        array_push($empty_plans, $floor);
                                                        continue;
                                                        echo 'No Units Available';
                                                    }
                                        ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php
                                                        $u = 0;
                                                        $c_unit = count($units);
                                                        // echo print_r($units);
                                                        $unit_counting = array();
                                                        while ($u < $c_unit) {
                                                            if (!empty($units[$u]->isAvailable)) {
                                                                array_push($unit_counting, '1')
                                                        ?>
                                                                <div class="details d-flex">
                                                                    <div class="price mx-2 strong">$ <?php echo $units[$u]->price; ?></div>
                                                                    <div class="apart mx-4">#<?php echo $units[$u]->aptNo; ?></div>
                                                                    <div class="apart mx-4">
                                                                        <?php
                                                                        // echo $units[$u]->moveIn . '<br/>';


                                                                        $date_now = time(); //current timestamp
                                                                        $date_convert = strtotime($units[$u]->moveIn);

                                                                        if ($date_now > $date_convert) {
                                                                            echo 'Now';
                                                                        } else {
                                                                            $newdate = date("M / d / Y", strtotime($units[$u]->moveIn));
                                                                            echo $newdate;
                                                                        }

                                                                        ?>
                                                                    </div>
                                                                </div>
                                                                <hr />
                                                        <?php
                                                                $u++;
                                                            } else {
                                                                $u++;
                                                                continue;
                                                            }
                                                        }
                                                        if (empty($unit_counting)) {
                                                            echo 'No Units Available';
                                                        }
                                                        ?>


                                                    </div>
                                                <?php
                                                }
                                            }
                                        }
                                        // echo print_r($empty_plans);
                                        foreach ($empty_plans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';
                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'studio') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }


                                                ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php

                                                        echo 'No Units Available';
                                                        ?>


                                                    </div>
                                        <?php
                                                }
                                            }
                                        }
                                        ?>
                                    </div>
                                    <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                                        <?php
                                        // echo print_r($floorplans); 
                                        $empty_plans = array();
                                        foreach ($floorplans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';
                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom1') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }
                                                    if (empty($unit_counting)) {
                                                        array_push($empty_plans, $floor);
                                                        continue;
                                                        echo 'No Units Available';
                                                    }

                                        ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php
                                                        $u = 0;
                                                        $c_unit = count($units);
                                                        // echo print_r($units);
                                                        $unit_counting = array();
                                                        while ($u < $c_unit) {
                                                            if (!empty($units[$u]->isAvailable)) {
                                                                array_push($unit_counting, '1')
                                                        ?>
                                                                <div class="details d-flex">
                                                                    <div class="price mx-2 strong">$ <?php echo $units[$u]->price; ?></div>
                                                                    <div class="apart mx-4">#<?php echo $units[$u]->aptNo; ?></div>
                                                                    <div class="apart mx-4">
                                                                        <?php
                                                                        // echo $units[$u]->moveIn . '<br/>';


                                                                        $date_now = time(); //current timestamp
                                                                        $date_convert = strtotime($units[$u]->moveIn);

                                                                        if ($date_now > $date_convert) {
                                                                            echo 'Now';
                                                                        } else {
                                                                            $newdate = date("M / d / Y", strtotime($units[$u]->moveIn));
                                                                            echo $newdate;
                                                                        }

                                                                        ?>
                                                                    </div>
                                                                </div>
                                                                <hr />
                                                        <?php
                                                                $u++;
                                                            } else {
                                                                $u++;
                                                                continue;
                                                            }
                                                        }
                                                        if (empty($unit_counting)) {
                                                            echo 'No Units Available';
                                                        }
                                                        ?>


                                                    </div>
                                                <?php
                                                }
                                            }
                                        }
                                        // echo print_r($empty_plans);
                                        foreach ($empty_plans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';
                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom1') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }


                                                ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                        $numbers2 = array_column($units, 'price');
                                                                        if (count(array_values(array_filter($numbers2))) > 0) {
                                                                    ?>
                                                                            <h4 class="price_low mb-5">$
                                                                                <?php
                                                                                $min2 = min(array_values(array_filter($numbers2)));
                                                                                $max2 = max(array_values(array_filter($numbers2)));
                                                                                echo $min2;
                                                                                ?> +


                                                                            </h4>
                                                                    <?php }
                                                                    } ?>

                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php

                                                        echo 'No Units Available';
                                                        ?>


                                                    </div>
                                        <?php
                                                }
                                            }
                                        }
                                        ?>
                                    </div>
                                    <div class="tab-pane fade" id="pills-contact" role="tabpanel" aria-labelledby="pills-contact-tab">
                                        <?php
                                        // echo print_r($floorplans); 
                                        $empty_plans = array();
                                        foreach ($floorplans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';

                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom2') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }
                                                    if (empty($unit_counting)) {
                                                        array_push($empty_plans, $floor);
                                                        continue;
                                                        echo 'No Units Available';
                                                    }
                                        ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php
                                                        $u = 0;
                                                        $c_unit = count($units);
                                                        // echo print_r($units);
                                                        $unit_counting = array();
                                                        while ($u < $c_unit) {
                                                            if (!empty($units[$u]->isAvailable)) {
                                                                array_push($unit_counting, '1')
                                                        ?>
                                                                <div class="details d-flex">
                                                                    <div class="price mx-2 strong">$ <?php echo $units[$u]->price; ?></div>
                                                                    <div class="apart mx-4">#<?php echo $units[$u]->aptNo; ?></div>
                                                                    <div class="apart mx-4">
                                                                        <?php
                                                                        // echo $units[$u]->moveIn . '<br/>';


                                                                        $date_now = time(); //current timestamp
                                                                        $date_convert = strtotime($units[$u]->moveIn);

                                                                        if ($date_now > $date_convert) {
                                                                            echo 'Now';
                                                                        } else {
                                                                            $newdate = date("M / d / Y", strtotime($units[$u]->moveIn));
                                                                            echo $newdate;
                                                                        }

                                                                        ?>
                                                                    </div>
                                                                </div>
                                                                <hr />
                                                        <?php
                                                                $u++;
                                                            } else {
                                                                $u++;
                                                                continue;
                                                            }
                                                        }

                                                        ?>


                                                    </div>
                                                <?php
                                                }
                                            }
                                        }
                                        // echo print_r($empty_plans);
                                        foreach ($empty_plans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';
                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom2') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }


                                                ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                        $numbers2 = array_column($units, 'price');
                                                                        if (count(array_values(array_filter($numbers2))) > 0) {
                                                                    ?>
                                                                            <h4 class="price_low mb-5">$
                                                                                <?php
                                                                                $min2 = min(array_values(array_filter($numbers2)));
                                                                                $max2 = max(array_values(array_filter($numbers2)));
                                                                                echo $min2;
                                                                                ?> +


                                                                            </h4>
                                                                    <?php }
                                                                    } ?>

                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php

                                                        echo 'No Units Available';
                                                        ?>


                                                    </div>
                                        <?php
                                                }
                                            }
                                        }
                                        ?>
                                    </div>
                                    <div class="tab-pane fade" id="pills-abcd" role="tabpanel" aria-labelledby="pills-abcd-tab">
                                        <?php
                                        // echo print_r($floorplans); 
                                        $empty_plans = array();
                                        foreach ($floorplans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';

                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom3') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }
                                                    if (empty($unit_counting)) {
                                                        array_push($empty_plans, $floor);
                                                        continue;
                                                        echo 'No Units Available';
                                                    }

                                        ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php
                                                        $u = 0;
                                                        $c_unit = count($units);
                                                        // echo print_r($units);
                                                        $unit_counting = array();
                                                        while ($u < $c_unit) {
                                                            if (!empty($units[$u]->isAvailable)) {
                                                                array_push($unit_counting, '1')
                                                        ?>
                                                                <div class="details d-flex">
                                                                    <div class="price mx-2 strong">$ <?php echo $units[$u]->price; ?></div>
                                                                    <div class="apart mx-4">#<?php echo $units[$u]->aptNo; ?></div>
                                                                    <div class="apart mx-4">
                                                                        <?php
                                                                        // echo $units[$u]->moveIn . '<br/>';


                                                                        $date_now = time(); //current timestamp
                                                                        $date_convert = strtotime($units[$u]->moveIn);

                                                                        if ($date_now > $date_convert) {
                                                                            echo 'Now';
                                                                        } else {
                                                                            $newdate = date("M / d / Y", strtotime($units[$u]->moveIn));
                                                                            echo $newdate;
                                                                        }

                                                                        ?>
                                                                    </div>
                                                                </div>
                                                                <hr />
                                                        <?php
                                                                $u++;
                                                            } else {
                                                                $u++;
                                                                continue;
                                                            }
                                                        }
                                                        if (empty($unit_counting)) {
                                                            echo 'No Units Available';
                                                        }
                                                        ?>

                                                    </div>
                                                <?php
                                                }
                                            }
                                        }
                                        // echo print_r($empty_plans);
                                        foreach ($empty_plans as $floor) {
                                            // echo print_r($floor) . '<br/><br/>';
                                            $units = $floor->units;
                                            if (count($units) > 0) {
                                                if ($floor->bed === 'bedroom3') {
                                                    $u = 0;
                                                    $c_unit = count($units);
                                                    // echo print_r($units);
                                                    $unit_counting = array();
                                                    while ($u < $c_unit) {
                                                        if (!empty($units[$u]->isAvailable)) {
                                                            array_push($unit_counting, '1');
                                                        }
                                                        $u++;
                                                    }


                                                ?>
                                                    <div class=" col-12 floorPlan p-3 mb-3">
                                                        <div class="d-flex justify-content-between flex-md-row flex-column">
                                                            <div>
                                                                <h2><?php echo $floor->name; ?> </h2>
                                                                <?php if (!empty($floor->plan2d)) { ?>
                                                                    <a class="popup-img d-block d-md-none" href="<?php echo $floor->plan2d; ?>">
                                                                        <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                    </a>
                                                                <?php } ?>
                                                                <div class="d-flex">
                                                                    <?php
                                                                    // echo print_r($floor);
                                                                    if (count($units) > 0) {
                                                                    ?>
                                                                        <h4 class="price_low mb-5">$
                                                                            <?php
                                                                            $numbers2 = array_column($units, 'price');
                                                                            $min2 = min(array_values(array_filter($numbers2)));
                                                                            $max2 = max(array_values(array_filter($numbers2)));
                                                                            echo $min2;
                                                                            ?> +


                                                                        </h4>
                                                                    <?php } ?>
                                                                    <h6 class="mx-3"><?php echo $floor->bed; ?></h6>
                                                                    <h6><?php echo $floor->bath; ?> Bath</h6>
                                                                </div>
                                                            </div>
                                                            <?php if (!empty($floor->plan2d)) { ?>
                                                                <a class="popup-img d-none d-md-block" href="<?php echo $floor->plan2d; ?>">
                                                                    <img src="<?php echo $floor->plan2d; ?>" alt="" class="img-fluid">
                                                                </a>
                                                            <?php } ?>
                                                        </div>
                                                        <?php

                                                        echo 'No Units Available';
                                                        ?>


                                                    </div>
                                        <?php
                                                }
                                            }
                                        }
                                        ?>
                                    </div>

                                </div>

                            </div>
                            <!-- <div class="floor-plan property wprt-image-video w50 pro">
                            <h5>What's Nearby</h5>
                            <div class="property-nearby">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="nearby-info mb-4">
                                            <span class="nearby-title mb-3 d-block text-info">
                                                <i class="fas fa-graduation-cap mr-2"></i><b class="title">Education</b>
                                            </span>
                                            <div class="nearby-list">
                                                <ul class="property-list list-unstyled mb-0">
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Education Mandarin</h6>
                                                        <span>(15.61 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Marry's Education</h6>
                                                        <span>(15.23 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">The Kaplan</h6>
                                                        <span>(15.16 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="nearby-info mb-4">
                                            <span class="nearby-title mb-3 d-block text-success">
                                                <i class="fas fa-user-md mr-2"></i><b class="title">Health & Medical</b>
                                            </span>
                                            <div class="nearby-list">
                                                <ul class="property-list list-unstyled mb-0">
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Natural Market</h6>
                                                        <span>(13.20 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Food For Health</h6>
                                                        <span>(13.22 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">A Matter of Health</h6>
                                                        <span>(13.34 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="nearby-info">
                                            <span class="nearby-title mb-3 d-block text-danger">
                                                <i class="fas fa-car mr-2"></i><b class="title">Transportation</b>
                                            </span>
                                            <div class="nearby-list">
                                                <ul class="property-list list-unstyled mb-0">
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Airport Transportation</h6>
                                                        <span>(11.36 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">NYC Executive Limo</h6>
                                                        <span>(11.87 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                    <li class="d-flex">
                                                        <h6 class="mb-3 mr-2">Empire Limousine</h6>
                                                        <span>(11.52 miles)</span>
                                                        <ul class="list-unstyled list-inline ml-auto">
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="fas fa-star-half fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                            <li class="list-inline-item m-0 text-warning"><i
                                                                    class="far fa-star fa-xs"></i></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div> -->
                            <!-- <div class="property wprt-image-video w50 pro">
                                <h5>Property Video</h5>
                                <img alt="image" src="images/slider/home-slider-4.jpg">
                                <a class="icon-wrap popup-video popup-youtube" href="https://www.youtube.com/watch?v=14semTlwyUY">
                                    <i class="fa fa-play"></i>
                                </a>
                                <div class="iq-waves">
                                    <div class="waves wave-1"></div>
                                    <div class="waves wave-2"></div>
                                    <div class="waves wave-3"></div>
                                </div>
                            </div> -->
                            <?php if (!empty($row['longitude'])) { ?>
                                <div class="property-location map">
                                    <h5>Location</h5>
                                    <div class="divider-fade"></div>
                                    <div id="map-contact" class="contact-map"></div>
                                </div>
                            <?php } ?>
                            <!-- Star Reviews -->
                            <!-- <section class="reviews comments">
                            <h3 class="mb-5">3 Reviews</h3>
                            <div class="row mb-5">
                                <ul class="col-12 commented pl-0">
                                    <li class="comm-inf">
                                        <div class="col-md-2">
                                            <img src="images/testimonials/ts-5.jpg" class="img-fluid" alt="">
                                        </div>
                                        <div class="col-md-10 comments-info">
                                            <div class="conra">
                                                <h5 class="mb-2">Mary Smith</h5>
                                                <div class="rating-box">
                                                    <div class="detail-list-rating mr-0">
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star-o"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <p class="mb-4">May 30 2020</p>
                                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras aliquam,
                                                quam congue dictum luctus, lacus magna congue ante, in finibus dui
                                                sapien eu dolor. Integer tincidunt suscipit erat, nec laoreet ipsum
                                                vestibulum sed.</p>
                                            <div class="rest"><img src="images/single-property/s-1.jpg"
                                                    class="img-fluid" alt=""></div>
                                        </div>
                                    </li>

                                </ul>
                            </div>
                            <div class="row">
                                <ul class="col-12 commented pl-0">
                                    <li class="comm-inf">
                                        <div class="col-md-2">
                                            <img src="images/testimonials/ts-4.jpg" class="img-fluid" alt="">
                                        </div>
                                        <div class="col-md-10 comments-info">
                                            <div class="conra">
                                                <h5 class="mb-2">Abraham Tyron</h5>
                                                <div class="rating-box">
                                                    <div class="detail-list-rating mr-0">
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <p class="mb-4">june 1 2020</p>
                                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras aliquam,
                                                quam congue dictum luctus, lacus magna congue ante, in finibus dui
                                                sapien eu dolor. Integer tincidunt suscipit erat, nec laoreet ipsum
                                                vestibulum sed.</p>
                                        </div>
                                    </li>

                                </ul>
                            </div>
                            <div class="row mt-5">
                                <ul class="col-12 commented mb-0 pl-0">
                                    <li class="comm-inf">
                                        <div class="col-md-2">
                                            <img src="images/testimonials/ts-3.jpg" class="img-fluid" alt="">
                                        </div>
                                        <div class="col-md-10 comments-info">
                                            <div class="conra">
                                                <h5 class="mb-2">Lisa Williams</h5>
                                                <div class="rating-box">
                                                    <div class="detail-list-rating mr-0">
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star"></i>
                                                        <i class="fa fa-star-o"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <p class="mb-4">jul 12 2020</p>
                                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras aliquam,
                                                quam congue dictum luctus, lacus magna congue ante, in finibus dui
                                                sapien eu dolor. Integer tincidunt suscipit erat, nec laoreet ipsum
                                                vestibulum sed.</p>
                                            <div class="resti">
                                                <div class="rest"><img src="images/single-property/s-2.jpg"
                                                        class="img-fluid" alt=""></div>
                                                <div class="rest"><img src="images/single-property/s-3.jpg"
                                                        class="img-fluid" alt=""></div>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </section> -->
                            <!-- End Reviews -->
                            <!-- Star Add Review -->
                            <!-- <section class="single reviews leve-comments details">
                            <div id="add-review" class="add-review-box">
                                
                                <h3 class="listing-desc-headline margin-bottom-20 mb-4">Add Review</h3>
                                <span class="leave-rating-title">Your rating for this listing</span>
                                
                                <div class="row mb-4">
                                    <div class="col-md-6">
                                        
                                        <div class="clearfix"></div>
                                        <div class="leave-rating margin-bottom-30">
                                            <input type="radio" name="rating" id="rating-1" value="1" />
                                            <label for="rating-1" class="fa fa-star"></label>
                                            <input type="radio" name="rating" id="rating-2" value="2" />
                                            <label for="rating-2" class="fa fa-star"></label>
                                            <input type="radio" name="rating" id="rating-3" value="3" />
                                            <label for="rating-3" class="fa fa-star"></label>
                                            <input type="radio" name="rating" id="rating-4" value="4" />
                                            <label for="rating-4" class="fa fa-star"></label>
                                            <input type="radio" name="rating" id="rating-5" value="5" />
                                            <label for="rating-5" class="fa fa-star"></label>
                                        </div>
                                        <div class="clearfix"></div>
                                    </div>
                                    <div class="col-md-6">
                                        
                                        <div class="add-review-photos margin-bottom-30">
                                            <div class="photoUpload">
                                                <span><i class="sl sl-icon-arrow-up-circle"></i> Upload Photos</span>
                                                <input type="file" class="upload" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12 data">
                                        <form action="#">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <input type="text" name="name" class="form-control"
                                                        placeholder="First Name" required>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <input type="text" name="name" class="form-control"
                                                        placeholder="Last Name" required>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <input type="email" name="email" class="form-control"
                                                        placeholder="Email" required>
                                                </div>
                                            </div>
                                            <div class="col-md-12 form-group">
                                                <textarea class="form-control" id="exampleTextarea" rows="8"
                                                    placeholder="Review" required></textarea>
                                            </div>
                                            <button type="submit" class="btn btn-primary btn-lg mt-2">Submit
                                                Review</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </section> -->
                            <!-- End Add Review -->
                        </div>
                        <aside class="col-lg-4 col-md-12 car">
                            <div class="single widget">
                                <!-- Start: Schedule a Tour -->
                                <div class="schedule widget-boxed mt-33 mt-0">
                                    <div class="widget-boxed-body pt-0 mt-0">
                                        <div class="sidebar-widget author-widget2">
                                            <div class="agent-contact-form-sidebar">


                                                <a href="connect_expert.php" class="mt-0 multiple-send-message">Connect with an expert</a>
                                            </div>
                                        </div>

                                        <!-- <div class="row mrg-top-15 mb-3">
                                            <div class="col-lg-6 col-md-12 mt-4">
                                                <label class="mb-4">Adult</label>
                                                <div class="input-group">
                                                    <span class="input-group-btn">
                                                        <button type="button" class="btn counter-btn theme-cl btn-number" disabled="disabled" data-type="minus" data-field="quant[1]">
                                                            <i class="fa fa-minus"></i>
                                                        </button>
                                                    </span>
                                                    <input type="text" name="quant[1]" class="border-0 text-center form-control input-number" data-min="0" data-max="10" value="0">
                                                    <span class="input-group-btn">
                                                        <button type="button" class="btn counter-btn theme-cl btn-number" data-type="plus" data-field="quant[1]">
                                                            <i class="fa fa-plus"></i>
                                                        </button>
                                                    </span>
                                                </div>
                                            </div>
                                            <div class="col-lg-6 col-md-12 mt-4">
                                                <label class="mb-4">Children</label>
                                                <div class="input-group">
                                                    <span class="input-group-btn">
                                                        <button type="button" class="btn counter-btn theme-cl btn-number" disabled="disabled" data-type="minus" data-field="quant[2]">
                                                            <i class="fa fa-minus"></i>
                                                        </button>
                                                    </span>
                                                    <input type="text" name="quant[2]" class="border-0 text-center form-control input-number" data-min="0" data-max="10" value="0">
                                                    <span class="input-group-btn">
                                                        <button type="button" class="btn counter-btn theme-cl btn-number" data-type="plus" data-field="quant[2]">
                                                            <i class="fa fa-plus"></i>
                                                        </button>
                                                    </span>
                                                </div>
                                            </div>
                                        </div> -->
                                        <!-- <a href="payment-method.php" class="btn reservation btn-radius theme-btn full-width mrg-top-10">Submit -->
                                        <!-- Request</a> -->
                                    </div>
                                </div>
                                <!-- End: Schedule a Tour -->
                                <!-- end author-verified-badge -->
                                <!-- <div class="sidebar">
                                    <div class="widget-boxed mt-33 mt-5">
                                        <div class="widget-boxed-header">
                                            <h4>Request Inquiry</h4>
                                        </div>


                                    </div>
                                </div> -->
                            </div>
                            <div class="main-search-field-2">
                                <!-- <div class="widget-boxed mt-5">
                                                <div class="widget-boxed-header">
                                                    <h4>Recent Properties</h4>
                                                </div>
                                                <div class="widget-boxed-body">
                                                    <div class="recent-post">
                                                        <div class="recent-main">
                                                            <div class="recent-img">
                                                                <a href="blog-details.php"><img
                                                                        src="images/feature-properties/fp-1.jpg" alt=""></a>
                                                            </div>
                                                            <div class="info-img">
                                                                <a href="blog-details.php">
                                                                    <h6>Family Home</h6>
                                                                </a>
                                                                <p>$230,000</p>
                                                            </div>
                                                        </div>
                                                        <div class="recent-main my-4">
                                                            <div class="recent-img">
                                                                <a href="blog-details.php"><img
                                                                        src="images/feature-properties/fp-2.jpg" alt=""></a>
                                                            </div>
                                                            <div class="info-img">
                                                                <a href="blog-details.php">
                                                                    <h6>Family Home</h6>
                                                                </a>
                                                                <p>$230,000</p>
                                                            </div>
                                                        </div>
                                                        <div class="recent-main">
                                                            <div class="recent-img">
                                                                <a href="blog-details.php"><img
                                                                        src="images/feature-properties/fp-3.jpg" alt=""></a>
                                                            </div>
                                                            <div class="info-img">
                                                                <a href="blog-details.php">
                                                                    <h6>Family Home</h6>
                                                                </a>
                                                                <p>$230,000</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div> -->
                                <!-- <div class="widget-boxed mt-5">
                                    <div class="widget-boxed-header ">
                                        <h4>Feature Properties</h4>
                                    </div>
                                    <div class="widget-boxed-body">
                                        <div class="owl-carousel featured">
                                            <?php
                                            $qry2 = "SELECT * FROM properties WHERE featured='1' ORDER BY RAND() LIMIT 3";
                                            $result2 = $conn->query($qry2);
                                            if ($result2->num_rows > 0) {
                                                // output data of each row
                                                while ($row2 = $result2->fetch_assoc()) {
                                                    $floorPlans = json_decode($row2['floorPlans']);
                                                    $imageUrls = json_decode($row2['imageUrls']);

                                                    if (is_array($floorPlans)) {
                                                        $count = count($floorPlans);
                                                        $i = 0;
                                                        if (!empty($floorPlans[0]->rentMin)) {
                                                            $ff =  '$' . $floorPlans[0]->rentMin . ' + ';
                                                        } else if (!empty($floorPlans[1]->rentMin)) {
                                                            $ff =  '$' . $floorPlans[1]->rentMin . ' + ';
                                                        } else if (!empty($floorPlans[2]->rentMin)) {
                                                            $ff =  '$' . $floorPlans[2]->rentMin . ' + ';
                                                        } else {
                                                            $ff =  'No Rent Info';
                                                        }


                                                        $types = array();
                                                        while ($i < $count && $i != 3) {
                                                            // echo $floorPlans[$i]->bed . '<span>|</span>';
                                                            if (in_array($floorPlans[$i]->bed, $types) === false) {
                                                                array_push($types, $floorPlans[$i]->bed);
                                                            }
                                                            $i++;
                                                        }
                                                        $tt = '';
                                                        foreach ($types as $t) {
                                                            if ($t === "bedroom1") {
                                                                $tt .=  '1 Bed' . ' | ';
                                                            } else if ($t === "bedroom2") {
                                                                $tt .=  '2 Bed' . ' | ';
                                                            } else if ($t === "bedroom3") {
                                                                $tt .=  '3 Bed' . ' | ';
                                                            } else {
                                                                $tt .=  $t . ' | ';
                                                            }
                                                        }
                                                    } else {
                                                        continue;
                                                    }
                                            ?>
                                                    <div class="agents-grid mr-0">
                                                        <div class="listing-item compact">
                                                            <a href="property_internal.php?id=<?php echo $row2['id']; ?>" class="listing-img-container">
                                                                <div class="listing-badges">
                                                                    <span class="featured"><?php echo $ff; ?></span>
                                                                </div>
                                                                <div class="listing-img-content">
                                                                    <span class="listing-compact-title">Property Id: <?php echo $row2['property_id']; ?></span>
                                                                    <ul class="listing-hidden-content">
                                                                        <?php echo $tt ?>
                                                                    </ul>
                                                                </div>
                                                                <img src="<?php if (!empty($imageUrls[0]) && $imageUrls[0] != 'https://api.sparkapt.com/rails/active_storage/representations/redirect/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBamx4IiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--fa1c42e16d700b720e4e26b13c477b0961f679ec/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdCem9MWm05eWJXRjBTU0lJY0c1bkJqb0dSVlE2RW5KbGMybDZaVjkwYjE5bWFYUmJCMmtDNkFNdyIsImV4cCI6bnVsbCwicHVyIjoidmFyaWF0aW9uIn19--43af89d2da5f0e3872ab5863ca5a5942f320921c/Pictures%20Coming%20Soon!.png') {
                                                                                echo $imageUrls[0];
                                                                            } else {
                                                                                echo './images/noImage.png';
                                                                            } ?>" alt="">
                                                            </a>
                                                        </div>

                                                    </div>
                                            <?php
                                                }
                                            }
                                            ?>

                                        </div>
                                    </div>
                                </div> -->
                                <!-- Start: Specials offer -->
                                <?php if (!empty($row['specials'])) { ?>
                                    <div class="widget-boxed popular mt-5">
                                        <div class="widget-boxed-header">
                                            <h4>Specials </h4>
                                        </div>
                                        <div class="widget-boxed-body">
                                            <div class="banner">
                                                <?php echo $row['specials']; ?>
                                            </div>
                                        </div>
                                    </div>
                                <?php
                                }
                                ?>
                                <div class="single homes-content details mb-30 mt-3">
                                    <!-- title -->
                                    <h5 class="mb-4">Property Details</h5>
                                    <ul class="homes-list clearfix">
                                        <li>
                                            <span class="font-weight-bold mr-1">Neighborhood:</span>
                                            <span class="det"><?php echo $row['neighborhood']; ?></span>
                                        </li>
                                        <li>
                                            <span class="font-weight-bold mr-1">City:</span>
                                            <span class="det"><?php echo $row['city']; ?></span>
                                        </li>

                                        <li>
                                            <span class="font-weight-bold mr-1">Year Built:</span>
                                            <span class="det"><?php echo $row['builtYear']; ?></span>
                                        </li>
                                        <!-- <li>
                                            <span class="font-weight-bold mr-1">Available From:</span>
                                            <span class="det"><?php
                                                                // $dates = array();
                                                                // foreach ($floorplans as $floor) {
                                                                //     echo $floor['availableFroms']
                                                                // }
                                                                ?></span>
                                        </li> -->
                                    </ul>
                                    <!-- title -->
                                    <h5 class="mt-5">Amenities</h5>
                                    <!-- cars List -->
                                    <ul class="homes-list clearfix">
                                        <?php
                                        $addonItems = json_decode($row['addonItems']);
                                        $count =  count($addonItems);
                                        $i = 0;
                                        while ($i < $count) {
                                        ?>
                                            <li>
                                                <i class="fa fa-check-square" aria-hidden="true"></i>
                                                <span><?php echo $addonItems[$i]; ?></span>
                                            </li>
                                        <?php
                                            $i++;
                                        }

                                        ?>
                                    </ul>
                                </div>
                                <!-- End: Specials offer -->
                                <!-- <div class="widget-boxed popular mt-5">
                                                <div class="widget-boxed-header">
                                                    <h4>Popular Tags</h4>
                                                </div>
                                                <div class="widget-boxed-body">
                                                    <div class="recent-post">
                                                        <div class="tags">
                                                            <span><a href="#" class="btn btn-outline-primary">Houses</a></span>
                                                            <span><a href="#" class="btn btn-outline-primary">Real
                                                                    Home</a></span>
                                                        </div>
                                                        <div class="tags">
                                                            <span><a href="#" class="btn btn-outline-primary">Baths</a></span>
                                                            <span><a href="#" class="btn btn-outline-primary">Beds</a></span>
                                                        </div>
                                                        <div class="tags">
                                                            <span><a href="#" class="btn btn-outline-primary">Garages</a></span>
                                                            <span><a href="#" class="btn btn-outline-primary">Family</a></span>
                                                        </div>
                                                        <div class="tags">
                                                            <span><a href="#" class="btn btn-outline-primary">Real
                                                                    Estates</a></span>
                                                            <span><a href="#"
                                                                    class="btn btn-outline-primary">Properties</a></span>
                                                        </div>
                                                        <div class="tags no-mb">
                                                            <span><a href="#"
                                                                    class="btn btn-outline-primary">Location</a></span>
                                                            <span><a href="#" class="btn btn-outline-primary">Price</a></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div> -->
                            </div>
                    </div>
                </div>
                </div>
                </aside>
                </div>

                </div>
            </section>

            <!-- END SECTION PROPERTIES LISTING -->

            <!-- START FOOTER -->
            <script>
                $('.popup-img').magnificPopup({
                    type: 'image',
                    disableOn: function() { // don't use a popup for mobile
                        if ($(window).width() < 600) {
                            return false;
                        }
                        return true;
                    },

                });
            </script>

<?php
            $js = '<script>
            
            if ($("#map-contact").length) {
                var map = L.map("map-contact", {
                    zoom: 16,
                    maxZoom: 50,
                    tap: false,
                    gestureHandling: true,
                    center: [' . $row["lat"] . ',' . $row["longitude"] . ']
                });
        
                map.scrollWheelZoom.disable();
        
                var Hydda_Full = L.tileLayer("https://{s}.tile.openstreetmap.de/tiles/osmde/{z}/{x}/{y}.png", {
                    scrollWheelZoom: false,
                    attribution: "&copy; <a href=`http://osm.org/copyright`>OpenStreetMap</a> contributors"
                }).addTo(map);
        
                var icon = L.divIcon({
                    html: `<i class="fa fa-building"></i>`,
                    iconSize: [50, 50],
                    iconAnchor: [50, 50],
                    popupAnchor: [-20, -42]
                });
        
                var marker = L.marker([' . $row["lat"] . ',' . $row["longitude"] . '], {
                    icon: icon
                }).addTo(map);
            }
        

            </script>        
            <script>
        $("#reservation-date").dateDropper();
       this.$("#reservation-time").timeDropper({
           setCurrentTime: false,
           meridians: true,
           primaryColor: "#e8212a",
           borderColor: "#e8212a",
           minutesInterval: "15"
       });
       $("#pills-tab li:first-child a").addClass("active")
   </script>';
        }
    }
}
include("footer.php");
?>